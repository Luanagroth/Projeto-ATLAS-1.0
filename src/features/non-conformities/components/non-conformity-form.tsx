...
export function NonConformityForm({
  ...
  const selectedAuditId = watch("auditId");
  ...
  function onSubmit(values: NonConformityFormValues) {
    ...
    startTransition(async () => {
      const result =
        isEdit && id
          ? await updateNonConformityAction({ id, values })
          : await createNonConformityAction(values);
  ...
  return (
    <form className="space-y-5 rounded-lg border bg-card p-6 shadow-sm" noValidate onSubmit={handleSubmit(onSubmit)}>
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div className="space-y-2 sm:col-span-2">
          ...
          <Textarea id="description" aria-invalid={Boolean(errors.description)} {...register("description")} />
          <FieldError message={errors.description?.message} />
        </div>

        <div className="space-y-2">
          <Label htmlFor="severity">Criticidade</Label>
          <select id="severity" className={cn("h-10 w-full rounded-lg border border-input bg-background px-3 text-sm")} {...register("severity")}>
        ...
        <div className="space-y-2">
          ...
          <select id="status" className={cn("h-10 w-full rounded-lg border border-input bg-background px-3 text-sm")} {...register("status")}>
        ...
        <div className="space-y-2">
          ...
          <select id="responsibleId" className={cn("h-10 w-full rounded-lg border border-input bg-background px-3 text-sm")} {...register("responsibleId")}>
      ...
      {errors.root ? <p className="rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">{errors.root.message}</p> : null}

      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
        <Button asChild type="button" variant="outline"><Link href={cancelHref}>Cancelar</Link></Button>
        <Button disabled={submitDisabled} type="submit">{submitDisabled ? "Salvando..." : isEdit ? "Salvar alterações" : "Criar não conformidade"}</Button>
      </div>
    </form>
  );
}
