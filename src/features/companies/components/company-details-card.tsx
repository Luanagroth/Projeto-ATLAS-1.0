    ...
    <dl className="grid gap-4 sm:grid-cols-2">
      {items.map((item) => (
        <div className="rounded-xl border bg-background/92 p-4" key={item.label}>
          ...
          <dd className="mt-1 text-sm leading-6">{formatValue(item.value)}</dd>
        </div>
      ))}
    </dl>
  );
...
export function CompanyDetailsCard({ company }: CompanyDetailsCardProps) {
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <DetailCard icon={MapPin} title="Localizacao">
        <DetailList
          items={[
      ...
      <DetailCard icon={BriefcaseBusiness} title="Informacoes operacionais">
        <DetailList
          items={[
            { label: "Ramo de atuacao", value: company.segment },
            { label: "Colaboradores", value: company.employeeCount },
            { label: "Descricao", value: company.description },
          ]}
        />
      </DetailCard>
