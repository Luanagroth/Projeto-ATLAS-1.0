...
export default async function CompanyPage({
  ...
  const [audits, evidences, history] = await Promise.all([
    listCompanyAudits({ companyId: company.id, organizationId: user.organizationId }),
    listCompanyEvidences({ companyId: company.id, organizationId: user.organizationId }),
    listCompanyHistory({ companyId: company.id, organizationId: user.organizationId }),
  ]);
  ...
  return (
    <section className="space-y-6">
      ...
      <CompanyDossierPanels
        audits={audits}
        canCreateAudit={canEditCompany}
        companyId={company.id}
        evidences={evidences}
        history={history}
      />
