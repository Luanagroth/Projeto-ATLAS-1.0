...
type NonConformityPageProps = {
  ...
  searchParams?: Promise<{ created?: string; resolved?: string; updated?: string }>;
};
    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col-reverse gap-2 sm:flex-row">
          <Button asChild variant="outline"><Link href="/non-conformities">Voltar</Link></Button>
          {hasRole(user, closerRoles) && item.status !== "RESOLVED" ? (
            <ResolveNonConformityButton id={item.id} />
          ) : null}
          {hasRole(user, editorRoles) ? (
            <Button asChild><Link href={`/non-conformities/${item.id}/edit`}>Editar</Link></Button>
          ) : null}
          {hasRole(user, deleteRoles) ? (
            <DeleteNonConformityButton id={item.id} title={item.title} />
          ) : null}
        </div>
      </div>
